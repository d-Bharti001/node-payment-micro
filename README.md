...
Every service has unit and integration tests ([Vitest](https://vitest.dev/)). The integration tests run against real dependencies started with [Testcontainers](https://testcontainers.com/): MySQL 8.4 with the real migrations applied, and a real [Mailpit](https://mailpit.axllent.org/) SMTP server for `email`. They exercise actual InnoDB locking and SMTP behavior instead of mocks. `api_gateway` needs no containers: its tests drive the real Express app, with only the two gRPC clients mocked.
...
| Service          | Highlights                                                                                                                                                                                                                                                                                                |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `money_movement` | Concurrent retries with one idempotency key produce a single debit, credit and outbox event. Opposing concurrent transfers neither deadlock nor lose money. Failures roll back completely. The outbox relay retries failed sends, and two relays never publish the same event (`FOR UPDATE SKIP LOCKED`). |
| `ledger`         | Redelivered events (including concurrent redelivery) are recorded once. The debit and credit are all-or-nothing. Malformed messages are skipped, not retried.                                                                                                                                             |
| `email`          | One event becomes one email for each side of the payment, delivered through a real SMTP server. Malformed events are skipped, SMTP failures are retried, and user-supplied text is HTML-escaped.                                                                                                          |
